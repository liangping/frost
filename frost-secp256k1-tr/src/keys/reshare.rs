...
mod tests {
    ...
    fn test_reshare() -> Result<(), crate::Error> {
        ...
        for new_index in 2..=max_signers + 1 {
            ...
            let new_identifier: Identifier = (new_index as u16).try_into().expect("should be nonzero");
            ...
            let old_pubkey= pubkey_packages.get(&new_identifier).expect("pub key should exist");
            assert_eq!(old_pubkey.verifying_key(), new_pubkeypackage.verifying_key(), "should have all verifying shares");

            refresh_key_packages.insert(new_identifier, new_keypackage);
            refresh_pubkey_packages.insert(new_identifier, new_pubkeypackage);
