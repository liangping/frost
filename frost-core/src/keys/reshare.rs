...
pub fn reconstruct_key_from_subshares<C: Ciphersuite>(helper_shares: BTreeMap<Identifier<C>, SecretShare<C>>, new_identifier: Identifier<C>, new_participants: &BTreeSet<Identifier<C>>) -> Result<(KeyPackage<C>, PublicKeyPackage<C>), Error<C>>{
    ...
    let group_commitments = crate::keys::sum_commitments(&commitments)?;
    ...
    if verifying_share.to_element() != public_key_package.verifying_shares().get(&new_identifier).unwrap().to_element() {
